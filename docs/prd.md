...
## Epic 4: Azure DevOps & Power Automate Integration

**Goal:** To create the Power Automate flows and configure the Azure DevOps board to act as our management dashboard, ensuring the state from our database is perfectly mirrored.

### Story 4.1: Create Power Automate Flows

**As a** system administrator,
**I want** to create Power Automate flows to connect Supabase to Azure DevOps
**so that** I can manage the state of the outreach funnel and track lead progression.

#### Acceptance Criteria

1.  The Power Automate flows must be configured to monitor the `status` column in the `leads` table.
2.  When a lead's status changes to `'Prospected'`, `'Engaged'`, or `'Messaged'`, the corresponding Power Automate flow must trigger.
3.  The flow must successfully move the correct Work Item on the Azure DevOps Kanban board to the column that matches the new status.
4.  A brief, automated comment (e.g., "Status updated to: Engaged") should be added to the Work Item's discussion history for traceability.
